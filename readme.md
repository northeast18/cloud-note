...
基于 Cloudflare Workers + D1 的网页版富文本与 Markdown 备忘录。密码登录、左列表右编辑、多端共享、便携分享，全程跑在 Cloudflare 免费额度内。

### ✨ 核心功能亮点
- 🔐 **真正的端到端加密 (E2EE)**：采用 Web Crypto (PBKDF2 + AES-GCM-256) 客户端加解密，服务器零知识。
- ♻️ **回收站与防误删机制**：软删除至回收站、一键恢复、彻底删除及一键清空回收站。
- 💾 **数据备份与一键恢复**：支持导出已解密的标准 JSON 备份，并支持从备份文件批量加密恢复到数据库。
- 🖼️ **前端图片 WebP 智能压缩**：客户端 Canvas 自动缩放限制并压缩为现代 WebP 格式上传 R2，极大节省存储与带宽。
- 📝 **Markdown / 富文本双模式**：每篇备忘录独立支持富文本或 Markdown，内置零依赖 Markdown 引擎，实时编辑与预览。
- ⚡ **智能无感自动保存**：2.5 秒输入防抖存盘、失焦存盘、切换笔记存盘与 30 秒后台巡检存盘。
- 🪟 **自研原生质感弹窗与 Toast**：取代系统原生弹窗，丝滑磨砂浮窗交互与状态气泡提示。
- 🔗 **分享生命周期管理**：独立“已分享”标签页，支持查看状态、一键复制、延长有效期（7天/30天/永久）、同步最新内容或取消分享。
- ⚙️ **偏好设置中心**：自定义默认笔记格式（富文本/Markdown）、图片缩放尺寸与压缩画质。
